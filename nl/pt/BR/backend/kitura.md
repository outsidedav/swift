...
O Kitura é capaz de se integrar a todos os serviços e recursos fornecidos pelo {{site.data.keyword.cloud}}, incluindo o {{site.data.keyword. appid_short}}, o {{site.data.keyword.mobilepushshort}} e o {{site.data.keyword.mobileanalytics_short}}, bem como bancos de dados, aprendizado de máquina e outros serviços. O Kitura pode, então, ser implementado e escalado automaticamente usando as plataformas Cloud Foundry ou Docker (baseadas no Kubernetes) no {{site.data.keyword.cloud}}.

O Kitura fornece uma [interface da linha de comandos (CLI)](http://www.kitura.io/en/starter/gettingstarted.html) do `kitura` que simplifica a criação, a construção, o teste e a implementação de aplicativos Kitura. Os aplicativos construídos usando a CLI do Kitura incluem suporte completo para implementação em qualquer nuvem que suporte tecnologias Cloud Foundry, Docker e Kubernetes. No entanto, se você estiver construindo especificamente para o {{site.data.keyword.cloud}}, recomenda-se usar o IBM Apple Development Console no navegador ou usar o {{site.data.keyword.dev_cli_notm}}. Além disso, enquanto ambos os métodos compartilham a tecnologia subjacente, o Apple Development Console e o IBM Developer Tools criam um projeto hospedado e um pipeline de implementação para você, bem como provisionam os serviços necessários para o seu aplicativo.
...
2. Forneça um nome ao seu projeto e selecione onde deseja implementá-lo. Se não tiver certeza de onde o aplicativo deve ser implementado, use os valores padrão, pois poderão ser mudados posteriormente.
3. Selecione o idioma Swift. Um projeto Swift do lado do servidor é criado. Também são exibidas as opções de Host e Domínio, que formam a URL para o aplicativo. Se não tiver certeza, use os padrões e também poderá fornecer seu próprio domínio customizado por meio de um provedor de domínio no qual o aplicativo deverá residir.
...
Um projeto é criado, mas um que ainda não usa serviços ou recursos adicionais. É possível incluir serviços usando o botão **Incluir recurso** ou clicando no botão **Fazer download do código** para obter o código do projeto. Também é possível incluir serviços facilmente em um projeto existente.

## Etapa 2. Incluindo Serviços

1. Clique no botão **Incluir recurso** para incluir serviços. Um painel de categorias de serviço é exibido. Por exemplo, selecione **Dados** para examinar os bancos de dados disponíveis e selecione **Cloudant NoSQL DB**.
2. Selecione um plano de precificação para o serviço, por exemplo, Lite, e clique em **Criar**.

Uma instância do serviço é criada e fornece credenciais de serviço para o aplicativo e, em alguns casos, inclui o código necessário em seu projeto para incluir a conexão correta com o serviço. Agora é possível incluir mais serviços usando o botão **Incluir recurso** ou clicando no botão **Fazer download do código** para obter o código do projeto.
...
1. Crie um projeto Xcode.  
  Antes que possa usar seu projeto no Xcode, deve-se criar um arquivo de projeto Xcode usando o comando do Swift Package Manager. Execute o comando a seguir do diretório-raiz de seu projeto transferido por download (que contém um arquivo `Package.swift`):
  ```
...
3. Execute o servidor Kitura localmente  
  Clique no botão **Executar** ou use o atalho de tecla `?+R` para iniciar o servidor Kitura. Depois que o servidor é iniciado, é possível verificar se as seguintes URLs padrão do Kitura estão em execução:
...
Um servidor Kitura de estrutura básica é criado, mas ele não fornece nenhuma API de REST que possa ser usada por um aplicativo iOS. Inclua APIs de REST no Kitura com a codificação mínima. As etapas a seguir mostram como incluir uma API de REST para a solicitação `GET` em `/meals`, que foi projetado para retornar os objetos `Meal` armazenados pelo servidor Kitura.
...
5. Execute o projeto.  
  Pressione o botão **Executar** ou use o atalho de tecla `?+R`. Se solicitado, selecione **Permitir conexões de rede recebidas**.

6. Teste a API de REST.
  Usando uma solicitação `GET`, que corresponde a como os navegadores da web solicitam dados de um servidor, é possível testar a API de REST usando a seguinte URL:  
  ```
...
1. Crie um Podfile na raiz do diretório de aplicativos iOS se ainda não tiver um:
  ```
  pod init
  ```
  {: codeblock}
...
## Próximas etapas
Agora que você tem um servidor Kitura que fornece uma API de REST que pode ser chamada por seu aplicativo iOS, você está pronto para implementar seu servidor no {{site.data.keyword.cloud_notm}}. As implementações podem ser feitas usando Contêineres com Kubernetes, Contêineres seguros ou o Cloud Foundry.
