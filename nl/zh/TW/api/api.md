...
您可以使用 API Connect 來管理 {{site.data.keyword.cloud}} 中的 API，無論該 API 是否在 {{site.data.keyword.cloud_notm}} 之外進行維護。學習管理您的 API，以便控制用量、提高採用率及追蹤統計資料。

## 建立 API Connect 實例

移至「型錄」，然後建立 API Connect 的實例，以管理您的 API。

使用 `Menu->APIs` 來存取「API Connect 管理」主控台。

![API Connect](images/apiconnect.png)

如果您在開始開發後端與前端系統之前先定義您自己的 API 合約，請使用 API Connect 工具來加速此處理程序。您可以與您的數位開發團隊合作，以在您的「iOS 應用程式」與後端邏輯之間，建置並定義 API 的合約。使用 [{{site.data.keyword.openwhisk}}](/docs/openwhisk/index.html)，或透過含 Kubernetes 或 [Cloud Foundry](/docs/cloud-foundry/index.html) 的 [Swift 執行時期](/docs/runtimes/swift/index.html)，即可分送此邏輯。
...
您可以定義 API Proxy，其在您的用戶端應用程式與後端邏輯之間，管理 API 閘道。使用下列步驟，利用您的 Open API Specification（Swagger 文件）YAML 或 JSON 來建立 Proxy。 

1. 開啟 `Menu -> APIs` 主控台，然後按一下 API Proxy。
2. 按一下 **API 定義匯入 YAML 或 JSON**。
3. 選取您之前建立的 YAML 或 JSON 檔。
4. 儲存並公開。

您需要配置「外部端點」，以指向鏈結至您後端邏輯應用程式的 URL。 

## 建立 Swift 後端

根據此 API 來建立後端 Swift 應用程式是有可能的。 

從 Apple Development 主控台，執行下列步驟：
...
然後您可以**下載**「程式碼」或**部署至 Cloud**，再將您的 GIT 報告複製到您的本端機器。您可以遵循 Knowledge Guide 中的指示，以 XCode 開啟伺服器端應用程式。

在**來源**資料夾中，您可以看到定義 Swift 檔的路徑，該檔案已建立對映至 API 的 REST 端點。 

請參閱下列範例，其使用 PetStore Open API：
```swift
import Kitura
...
使用 {{site.data.keyword.openwhisk_short}} 或完整堆疊 Swift 運行環境來定義 API，並建立 API Connect 定義之後，您便可耗用「iOS 應用程式」中的 API。

## 耗用 iOS 應用程式行動應用程式中的 API

若要耗用「iOS 應用程式」中的後端 API，請使用 Apple Console 來建立「行動」入門範本套件。使用「入門範本套件」視圖，建立任何類型的 iOS Swift 入門範本套件。

按一下**新增資源**，然後選取 API。 

![API 對話](../images/apidialog.png)

API 已新增至您的「iOS 應用程式」。如果您*下載* 應用程式的程式碼，您會看到一個以 API 命名的資料夾內含在「iOS 來源」資料夾中。

請遵循 Knowledge Guide 步驟，以 `pod update` 任何相依 SDK 至您的「iOS 應用程式」。 

「iOS 應用程式」包含一個資料夾，其中包含 API 的已產生 SDK 連結。此資料夾包含下列 3 個子資料夾：`Assets`、`Source` 及 `Docs`。 

![iOS 資料夾](../images/sdkfolder.png)

`Assets` 資料夾包含的檔案，用來管理連至 API 的 URL，依預設為 `localhost:3000`。您必須將此值變更成參照「API 路徑」。API 定義包含一個「API 名稱及路徑」區段。按一下路徑尾端的**複製圖示**，以複製 URL。檢查*公開受管理 API* 選項是否已開啟，以便讓外部用戶端可以進行 API 呼叫。

![API 路徑](../images/apiroute.png)  

開啟 `PLIST` 檔，將主機值取代為從 API 路徑中複製的值，使 SDK 可以將 API 呼叫至 {{site.data.keyword.cloud_notm}}。

## 文件

當 SDK 內含在您的「iOS 應用程式」專案中時，**Docs 資料夾**中會有一個*README.html* 檔。以「外部」瀏覽器開啟 Docs 資料夾，並閱讀如何使用專案的相關指示。

## API 變更之後重建 SDK

如果 API 變更或有新的特性可用，且已新增 {{site.data.keyword.openwhisk}}，您可以使用 `ibmcloud sdk` 指令來重建 Client SDK。如需相關資訊，範例及語法說明，請查看 [SDK 產生器](/docs/cli/sdk/index.html)文件。
...
4. 選取下載圖示，以下載 API 的 YAML，並將此檔案儲存在您的「iOS 應用程式」專案目錄中。
...
5. 下一步是執行 `ibmcloud sdk` CLI 指令。
    ...
    SDK 已重建至您的「iOS 應用程式」專案目錄，因此您可以繼續使用 API。

## 參考資料

下列範例 SDK 是針對 {{site.data.keyword.openwhisk_short}} 從「入門範本套件」建立的。您可以看到可包含在您「iOS 應用程式」中的每個「動作」以及程式碼的 Swift Snippet。
