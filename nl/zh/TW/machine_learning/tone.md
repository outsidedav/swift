...
使用您最愛的文字編輯器，在您專案的根目錄（`.xcodeproj` 檔所在之處）中，建立一個稱為 `Cartfile` 的新檔案。然後，新增一行，指定 Watson Swift SDK 作為相依關係：
...
若為正式作業應用程式，您也可能想要指定特定的[版本需求](https://github.com/Carthage/Carthage/blob/master/Documentation/Artifacts.md#version-requirement)，以避免新版本 SDK 有非預期的變更。

您的 `Cartfile` 就定位之後，現在您可以下載並建置相依關係。使用終端機來導覽至您專案的根目錄，然後執行 Carthage：
...
現在，Carthage 已建置 Watson Swift SDK 架構，您必須鏈結「語氣分析器」架構與您的應用程式。
...
除了_鏈結_「語氣分析器」架構，您也必須將其_複製_ 到應用程式中，才能在執行時期進行存取。Xcode 有數個不同的方法可複製或內含架構，但是您可以使用 Carthage Script 來避免特定的 [App Store 提交錯誤](http://www.openradar.me/radar?id=6409498411401216)。
...
4. 將「語氣分析器」架構新增至「輸入檔」清單：`$(SRCROOT)/Carthage/Build/iOS/ToneAnalyzerV3.framework`。
...
4. 將下列程式碼新增至 `toneAnalyzerExample` 函數。請務必更新您服務的使用者名稱及密碼。
  ```swift
  import UIKit
  import ToneAnalyzerV3
...
[入門範本套件](https://console.bluemix.net/developer/appledevelopment/starter-kits)是利用 {{site.data.keyword.cloud_notm}} 功能最快的方式之一。選取 **Tone Analyzer for iOS with Watson** 入門範本套件，便可使用 {{site.data.keyword.toneanalyzershort}} 服務。此服務利用深度學習功能來評估文字通道。「語氣分析器」應用程式會識別說話者的語氣（快樂、悲傷、自信等等），因為它與許多種類有關。
