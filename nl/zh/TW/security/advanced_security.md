...
Swift 開發人員可以輕易地利用 {{site.data.keyword.cloud}} 所提供的進階安全服務，以業界最高的安全層次保護不在使用中、使用中或傳輸中的金鑰及資料。
{:shortdesc}
...
{{site.data.keyword.hscrypto}} 是一個實驗性服務，對您的金鑰及資料提供業界最高安全層次的加密法。它將 System z 的安全與完整性帶至雲端。現在則透過 {{site.data.keyword.cloud_notm}}，將銀行及金融服務所仰賴，且同樣使用最先進技術的加密技術，提供給雲端使用者。

{{site.data.keyword.hscrypto}} 提供可網路定址的硬體安全模組 (HSM)，透過 PKCS#11 應用程式設計介面 (API)，提供安全且受保護的加密法。對於加密硬體，您可以存取可實現的最高安全層次，也就是「FIPS 140-2 層次 4」。{{site.data.keyword.hscrypto}} 也利用可遠端存取 IBM 加密輔助處理器的 IBM Advanced Crypto Service Provider (ACSP) 解決方案。
...
{{site.data.keyword.keymanagementserviceshort}} 可協助您在 {{site.data.keyword.cloud_notm}} 服務之間為應用程式佈建加密金鑰。當您管理金鑰的生命週期時，便可知道您的金鑰是由防止資訊竊取的雲端型硬體安全模組 (HSM) 所保護。搭配使用 {{site.data.keyword.hscrypto}}，您的金鑰便是處於最高安全層次「FIPS 140-2 層次 4」憑證的保護傘下了。
...
{{site.data.keyword.cloud_notm}} Hyper Protect DBaaS 是一個實驗性 {{site.data.keyword.cloud_notm}} 服務，隨需提供安全的資料庫。其提供彈性且可擴充的平台，可快速且輕易地佈建及管理您的 MongoDB 資料庫。
 ...
 * 基礎技術防止 {{site.data.keyword.IBM_notm}} 或協力廠商存取您的資料。

### 新增 {{site.data.keyword.cloud_notm}} Hyper Protect DBaaS 邏輯至應用程式
...
在您變成贊助者使用者之前，您可以使用 {{site.data.keyword.containershort_notm}} 來保護您的應用程式。如需相關資訊，請參閱[開始使用 {{site.data.keyword.containershort_notm}}](/docs/containers/container_index.html#container_index)。
