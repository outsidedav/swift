...
[Kitura](http://www.kitura.io) è un framework Swift lato server per creare applicazioni web e backend iOS. Questo framework crea delle API REST che possono essere richiamate dall'applicazione iOS utilizzando gli SDK URLSession quali Alamofire, RestKit oppure l'SDK [KituraKit](https://github.com/ibm-swift/kiturakit) fornito da Kitura stesso.

Kitura è in grado di integrarsi con tutti i servizi e tutte le funzionalità forniti da {{site.data.keyword.cloud}}, compresi {{site.data.keyword. appid_short}}, {{site.data.keyword.mobilepushshort}} e {{site.data.keyword.mobileanalytics_short}}, così come con database, machine learning e altri servizi. Kitura può quindi essere distribuito e ridimensionato automaticamente utilizzando le piattaforme Cloud Foundry o Docker (basata su Kubernetes) in {{site.data.keyword.cloud}}.

Kitura fornisce una `kitura` [CLI (command line interface)](http://www.kitura.io/en/starter/gettingstarted.html) che semplifica la creazione, lo sviluppo, la verifica e la distribuzione di applicazioni Kitura. Le applicazioni sviluppate utilizzando la CLI Kitura includono il pieno supporto per la distribuzione a qualsiasi cloud che supporta le tecnologie Cloud Foundry, Docker e Kubernetes. Tuttavia, se stai eseguendo attività di creazione specificamente per {{site.data.keyword.cloud}}, ti consigliamo di utilizzare la IBM Apple Development Console nel browser o di utilizzare la {{site.data.keyword.dev_cli_notm}}. Inoltre, mentre entrambi i metodi condividono la tecnologia sottostante, la Apple Development Console e IBM Developer Tools creano per tuo conto un progetto ospitato e una pipeline di distribuzione, oltre ad eseguire il provisioning dei servizi di cui la tua applicazione ha bisogno.
...
## Passo 1. Creazione di un progetto Kitura utilizzando il browser

1. Vai alla sezione Kit starter della Apple Development Console. Seleziona uno starter predefinito, come ad esempio "Swift for Backend for Frontend API" oppure crea un progetto personalizzato utilizzando il tile **Create project** . Fai clic su **Create Project**.
2. Dai al tuo progetto un nome e seleziona dove vuoi che venga distribuito il tuo progetto. Se non sei sicuro di dove deve essere distribuita l'applicazione, utilizza i valori predefiniti poiché è possibile modificarli in un secondo momento. 
3. Seleziona il linguaggio Swift. Viene creato un progetto Swift lato server. Sono visualizzate anche le opzioni Host e Domain, che formano l'URL per l'applicazione. Se non sei sicuro, utilizza i valori predefiniti; puoi anche fornire un tuo dominio personalizzato da un provider di domini dove l'applicazione deve risiedere.
...
Viene creato un progetto che però non utilizza ancora alcun servizio o alcuna funzionalità aggiuntivi. Puoi aggiungere dei servizi utilizzando il pulsante **Add Resource** oppure facendo clic sul pulsante **Download Code** per ottenere il codice per il progetto. Puoi anche aggiungere facilmente dei servizi a un progetto esistente.

## Passo 2. Aggiunta di servizi

1. Fai clic sul pulsante **Add Resource** per aggiungere dei servizi. Viene visualizzato un pannello di categorie di servizi. Ad esempio, seleziona **Data** per visualizzare i database disponibili e seleziona **Cloudant NoSQL DB**.
2. Seleziona un piano di prezzi per il servizio, ad esempio Lite, e fai clic su **Create**.

Viene creata un'istanza del servizio che ti fornisce le credenziali di servizio per l'applicazione e, in alcuni casi, aggiunge il codice necessario al tuo progetto per includere la corretta connessione al servizio. Puoi ora aggiungere ulteriori servizi utilizzando il pulsante **Add Resource** o facendo clic sul pulsante **Download Code** per ottenere il codice per il progetto.

Dopo che hai scaricato il tuo progetto, puoi iniziare a lavorare con la tua applicazione.

## Passo 3. Sviluppo della tua applicazione con Xcode
Dopo che hai scaricato il tuo progetto, puoi modificarlo e svilupparlo utilizzando Xcode e caricare quindi l'applicazione modificata per la distribuzione al cloud.

1. Crea un progetto Xcode.  
  Prima di poter utilizzare il tuo progetto in Xcode, devi creare un file di progetto Xcode utilizzando il comando del gestore pacchetti Swift. Esegui il seguente comando dalla directory root del progetto scaricato (che contiene un file `Package.swift`):
  ```
...
3. Esegui il server Kitura localmente  
  Fai clic sul pulsante **Run** oppure utilizza la scelta rapida da tastiera `⌘+R` per avviare il server Kitura. Una volta avviato il server, puoi controllare che i seguenti URL Kitura standard siano in esecuzione.
  * Kitura Monitoring: [http://localhost:8080/swiftmetrics-dash/]()
  * Kitura Health check: [http://localhost:8080/health]()

## Passo 5. Aggiunta di API REST
Viene creato un server Kitura di base che però non fornisce alcuna API REST che può essere utilizzata da un'applicazione iOS. Aggiungi le API REST in Kitura con una codifica minima. I seguenti passi mostrano come aggiungere una API REST per la richiesta `GET` su `/meals`, che è progettata per restituire gli oggetti `Meal` archiviati dal server Kitura.
...
5. Esegui il progetto.  
  Premi il pulsante **Run** oppure utilizza la scelta rapida da tastiera `⌘+R`. Se ti viene richiesto, seleziona **Allow incoming network connections**.

6. Verifica l'API REST.
  Utilizzando una richiesta `GET`, che corrisponde al modo in cui i browser web richiedono dati da un server, puoi verificare l'API REST utilizzando il seguente URL:  
  ```
...
Puoi utilizzare l'esercitazione [FoodTrackerBackend](https://github.com/IBM/FoodTrackerBackend), che ti aiuta a creare una serie di API REST per l'archiviazione, il recupero e l'eliminazione di oggetti `Meal` da un'applicazione iOS, compresa l'archiviazione dei dati in un database.
{: tip}

## Passo 6. Installazione di KituraKit nella tua applicazione iOS
...
## Passi successivi
Ora che hai un server Kitura che fornisce una API REST che può essere richiamata dalla tua applicazione iOS, sei pronto a distribuire il tuo server a {{site.data.keyword.cloud_notm}}. Le distribuzioni possono essere eseguite utilizzando i contenitori con Kubernetes, i contenitori protetti oppure Cloud Foundry.
