...
1. Decomprimi il codice scaricato. Quindi, utilizzando un terminale, passa alla cartella estratta.
  ```
  cd <Name of Project>
  ```
...
1. Apri `.xcworkspace` in Xcode e passa a `AppDelegate.swift`.
  **Nota:** assicurati di aprire sempre il nuovo spazio di lavoro Xcode, invece del file di progetto Xcode originale: `MyApp.xcworkspace`.
  ...
  `BMSCore` è l'SDK principale ed è la base per gli SDK client mobili. `BMSClient` è una classe di `BMSCore`, inizializzata in `AppDelegate.swift`. Insieme a `BMSCore`, l'SDK {{site.data.keyword.mobileanalytics_short}} è già importato nel progetto.
...
Il servizio include la console {{site.data.keyword.mobileanalytics_short}}, dove gli sviluppatori e i proprietari delle applicazioni possono monitorare le prestazioni dell'applicazione mobile, visualizzare le statistiche di utilizzo e cercare nei log dispositivo.
...
Puoi anche imparare a sviluppare applicazioni Swift utilizzando gli [strumenti per gli sviluppatori {{site.data.keyword.cloud_notm}}](../cli/index.html), che offrono un approccio da riga di comando per la creazione, lo sviluppo e la distribuzione di applicazioni del microservizio, mobili e web complete.
