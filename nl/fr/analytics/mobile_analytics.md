...
{{site.data.keyword.mobileanalytics_short}} sur {{site.data.keyword.cloud_notm}} permet aux développeurs, aux administrateurs informatiques et aux parties prenantes de prendre connaissance des performances de leurs applis et de savoir comment elles sont utilisées. Grâce au service {{site.data.keyword.mobileanalytics_short}}, vous pouvez :

 - **Prendre immédiatement connaissance** : consultez en temps réel les métriques de performance et les statistiques d'utilisation.

 - **Procéder à une mise en oeuvre en quelques minutes** : créez une instance de service dans {{site.data.keyword.cloud_notm}}, ajoutez le logiciel SDK à votre projet, collez deux lignes de code dans votre application et vous voilà prêt(e) pour la collecte de douzaines de métriques prédéfinies.

 - **Collecter les données de votre choix** : Instrumentez les applis à l'aide d'événements personnalisés, découvrez comment les utilisateurs interagissent avec votre appli, suivez les achats et surveillez l'activité de l'appli.

 - **Consulter un aperçu des métriques ** : la console {{site.data.keyword.mobileanalytics_short}} propose des graphiques déjà tout prêts, ne nécessitant pas l'écriture de requêtes.

 - **Vous concentrer sur ce qui est important pour vous** : filtrez les métrique par heure, adaptateur, application, version d'application, SE, version de SE ou modèle d'appareil.

 - **Identifier rapidement les problèmes ** : surveillez le statut des pannes. Définissez des déclencheurs d'alerte sur des mesures critiques et dirigez les alertes vers n'importe quel noeud final REST.

 - **Dépanner la cause première ** : utilisez la journalisation personnalisée dans votre application et chargez automatiquement les journaux et effectuez des recherches dans ces derniers depuis la console. Explorez en aval les événements de panne pour voir les traces de pile.
...
2. Activez l'API d'emplacement en ajoutant une propriété dans le fichier `Info.plist` du dossier projet de votre appli. Par exemple, entrez `Privacy - Location Usage Description` et indiquez une justification pour l'ajout de l'API d'emplacement, par exemple "L'appli exige que le service de localisation soit activé".
...
1. Suivez les [instructions du logiciel SDK {{site.data.keyword.Bluemix_notm}} Mobile Services![Icône de lien externe](../../icons/launch-glyph.svg "Icône de lien externe")](https://github.com/ibm-bluemix-mobile-services/bms-clientsdk-swift-analytics/tree/development#cocoapods){: new_window} sur GitHub pour installer `BMSAnalytics` à l'aide de Cocoapods et ajoutez-le à votre fichier Pod.

2. Après installation du logiciel SDK iOS Client, [importez et initialisez](sdk.html#initalize-ma-sdk) le logiciel SDK Analytics Client.   
...
2. Après installation du logiciel SDK iOS Client, importez, puis initialisez le logiciel SDK Analytics Client.
...
La méthode `Analytics.send()` remplit les tableaux **Vue d'ensemble des pannes** et **Pannes** de la page **Pannes**. Les graphiques sont activés à l'aide du processus d'initialisation et d'envoi pour analyse ; aucune configuration spéciale n'est nécessaire.
...
## Etape 12. Test de votre appli
{: #appid_testing}

Est-ce que tout est correctement configuré ? Il est temps de tester !

1. Ouvrez votre appli. Si vous disposez d'une application Web, utilisez un navigateur. Si vous disposez d'une application client iOS, utilisez l'émulateur Xcode.
...
{: #what-to-do-next notoc}
 ...
 - Pour une présentation de l'utilisation des services mobiles et d'{{site.data.keyword.Bluemix_notm}}, voir [Mise en route avec les applis mobiles sur IBM Cloud](/docs/services/mobile/index.html).

 - Les Kits de démarrage sont des moyens rapides d'optimiser les fonctionnalités d'{{site.data.keyword.cloud_notm}}. La liste des kits de démarrage disponibles est accessible sur le [tableau de bord du développeur d'applications mobiles](https://console.bluemix.net/developer/mobile/dashboard). Téléchargez le code. Exécutez l'appli.
