...
3. プロジェクトに名前を付け、**「作成」**をクリックします。
  ...
  ストレージが定義されていない場合は、「更新」をクリックします。{: tip}
...
これで、準備が完了しました。 これで、[Watson Developer Cloud Swift SDK](https://github.com/watson-developer-cloud/swift-sdk){:new_window} を使用して Core ML モデルをダウンロードし、アプリに統合する準備ができました。
...
2. API 鍵とバージョンを渡して、SDK を初期化します。
  ...
  [バージョン・パラメーターの資料](https://cloud.ibm.com/apidocs/visual-recognition#versioning)を確認するか、{site.data.keyword.visualrecognitionshort}} サービスが作成された日付を使用してください。{: tip}
...
5. Watson SDK でサポートされているその他の [Core ML 分類機能](https://watson-developer-cloud.github.io/swift-sdk/services/VisualRecognitionV3/index.html)を確認します。
...
* {{site.data.keyword.visualrecognitionshort}} が提供するすべての機能を利用する。詳しくは、[ドキュメンテーション](/docs/services/visual-recognition/index.html)を参照してください。
