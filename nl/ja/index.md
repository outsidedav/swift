...
{{site.data.keyword.cloud}} は、皆様の顧客が求めるセキュリティー、AI、価値を Swift 開発者およびアプリケーションが実現できるようにするためのソリューションとサービスを提供します。幅広いオファリングのポートフォリオや SDK が用意されているので、これらのサービスを利用して最先端のアプリケーションを迅速にリリースすることが可能になります。Swift プログラミング・ガイドは、新規または既存の Swift アプリケーションにサービスを追加する方法について説明します。iOS クライアントであるかサーバー・サイド Swift であるかは関係ありません。{: shortdesc}

以下のチュートリアルは、[{{site.data.keyword.cloud_notm}} Developer Console for Apple](https://console.bluemix.net/developer/appledevelopment/starter-kits) の空のスターター・キットを使用して、{{site.data.keyword.mobileanalytics_full}} で簡単に Swift モバイル・アプリを作成する際のエントリー・ポイントになります。{{site.data.keyword.mobileanalytics_short}} サービスを追加したり、コードをダウンロードしたり、iOS アプリを Xcode を使用してローカルで実行したり、アプリを構成したり、アプリをモニターしたりといった作業を、コンソールから行います。
...
1. ダウンロードしたコードを unzip します。次に、端末を使用して unzip したフォルダーにナビゲートします。
  ```
  cd <プロジェクト名>
  ```
...
1. Xcode で `.xcworkspace` を開き、`AppDelegate.swift` にナビゲートします。**注:** 元の Xcode プロジェクト・ファイルではなく、次のように必ず新しい Xcode ワークスペースである `MyApp.xcworkspace` を開いてください。
...
3. `logger` を使用して使用分析を収集します。`ViewController.swift` ファイルにナビゲートして、以下のコードを表示します。
  ...
  {: codeblock}
   ...
   高度な分析およびロギング機能については、[使用分析の収集](https://console.bluemix.net/docs/services/mobileanalytics/sdk.html#app-monitoring-gathering-analytics)および[ロギング](https://console.bluemix.net/docs/services/mobileanalytics/sdk.html#enabling-configuring-and-using-logger)を参照してください。{:tip}
...
* [プッシュ通知サービスの追加](/push/push_notifications.html)
* [アプリ ID を使用したユーザー認証の追加](/authenticate/app_id.html)

### {{site.data.keyword.cloud_notm}} 開発者ツールの使用
また、[{{site.data.keyword.cloud_notm}} 開発者ツール](../cli/index.html)を使用して、Swift アプリの開発方法を学習することもできます。このツールは、エンドツーエンド Web アプリケーション、モバイル・アプリケーション、およびマイクロサービス・アプリケーションの作成、開発、およびデプロイを行うためのコマンド行アプローチを提供します。
