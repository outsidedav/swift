...
`Assets` 폴더에는 API에 대한 URL을 관리하는 파일이 포함되어 있으며, 기본값은 `localhost:3000`입니다. API 라우트를 참조하려면 값을 변경해야 합니다. API 정의는 API 이름 및 라우트 섹션으로 구성됩니다. 라우트의 끝에 있는 **복사**를 클릭하여 URL을 복사하십시오. API를 호출하기 위해 외부 클라이언트를 사용으로 설정하려면 *관리 API 노출* 옵션이 켜져 있는지 확인십시오.
