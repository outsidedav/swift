...
준비 검사는 다운스트림 서비스가 사용 불가능한 경우 허용 가능한 폴백이 없을 때 결과에서 다운스트림 서비스에 대한 연결의 실행 가능성을 포함해야 합니다. 이는 인프라가 사용자를 위해 확인함에 따라 다운스트림 서비스에서 직접 제공하는 상태 검사의 호출을 의미하지는 않습니다. 대신 애플리케이션에 있는 다운스트림 서비스에 대한 기존 참조의 상태 확인을 고려하십시오. 이는 WebSphere MQ에 대한 JMS 연결 또는 초기화된 Kafka 이용자 또는 생성자일 수 있습니다. 다운스트림 서비스에 대한 내부 참조의 실행 가능성을 확인하는 경우 애플리케이션에 있는 영향 상태 검사를 최소화하도록 결과를 캐시하십시오.
...
**Codable** 또는 표준 사전 사용과 같은 대체 구현의 경우 [Health 라이브러리 예제](https://github.com/IBM-Swift/Health#usage){: new_window} ![외부 링크 아이콘](../../icons/launch-glyph.svg "외부 링크 아이콘")를 참조하십시오. 이 구현 중 일부는 서비스 지원을 위해 수행되는 캐싱 검사에 대한 지원을 통해 확장 가능한 상태 검사의 작성을 간소화합니다. 이 시나리오에서 사용자는 단순한 라이브니스 테스트를 더욱 강력하고 세부적인 준비 검사와 구분하기를 원합니다.
...
자세한 정보는 [라이브니스 및 준비 프로브 구성](https://kubernetes.io/docs/tasks/configure-pod-container/configure-liveness-readiness-probes/){: new_window} ![외부 링크 아이콘](../../icons/launch-glyph.svg "외부 링크 아이콘") 방법을 참조하십시오.
