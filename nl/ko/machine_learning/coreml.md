...
4. **코드 다운로드**를 클릭하여 프로젝트를 다운로드하십시오. iOS 프로젝트의 경우 신임 정보가 해당 키 필드의 `BMSCredentials.plist` 파일에 삽입됩니다. 서버 측 Swift 프로젝트의 경우 `config/local-dev.json` 파일에서 이 신임 정보를 찾을 수 있습니다. 
